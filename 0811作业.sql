...
-- 查找所有以’b’字母开头的水果
SELECT * FROM fruits WHERE f_price like 'b%';
-- 在fruits表中，查询f_name中包含字母’g’的记录
SELECT * FROM fruits WHERE f_name regexp 'g';
...
SELECT *FROM employee e WHERE e.e_salary =
...
SELECT * FROM employee WHERE (datediff(curdate(now()),hireDate))>10;
...
SELECT f_name,f_price FROM fruits WHERE f_id=101 and f_price>=5;
...
-- 查询s_id=101或者s_id=102的水果供应商的f_price和f_name
SELECT f_price,f_name FROM fruits WHERE s_id between 101 and 102;
...
SELECT f_name,f_price FROM fruits order by f_price;
...
SELECT s_id,f_name FROM fruits group by s_id;
...
SELECT o_item,quantity*item_price FROM orderitems WHERE quantity*item_price>100; 
-- 将以上内容使用ORDER BY关键字按总订单价格排序显示结果
SELECT quantity*item_price FROM orderitems order by quantity*item_price desc;
-- 显示fruits表查询结果的前4行
...
SELECT o_item count(*) FROM orderitems group by o_item;
...
SELECT f_name,max(f_price) from fruits;
-- 在fruits表中查找不同供应商提供的价格最高的水果
SELECT max(f_price),s_id FROM fruits group by s_id;
-- 在fruits表中查找f_name的最大值
SELECT max(f_name) FROM fruits;
-- 在fruits表中查找市场上价格最低的水果
SELECT min(f_price) FROM fruits;
-- 在fruits表中查找不同供应商提供的价格最低的水果
SELECT min(f_price),s_id FROM fruits group by s_id;
