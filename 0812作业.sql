...
alter table tb_emp add constraint fk_tb_emp_id foreign key (deptId) references tb_dept(id);
...
(
    ...
    deptId INT(11),
    salary FLOAT,
    constraint fk_tb_emp_id1 foreign key (deptId) references tb_dept(id)
);
...
alter table tb_emp drop foreign key fk_tb_emp_id;
alter table tb_emp add constraint fk_tb_emp_id foreign key (deptId) references tb_dept1(id);
...
where suppliers.s_id=(select fruits.s_id from fruits where f_id='a1') 
and not f_id='a1';
...
select fruits.* ,suppliers.s_name 
...
select o.o_date from orders
as o where o.o_num=30001;
...
inner join score on student.id=stu_id group by name;
-- 计算每个考试科目的平均成绩
select c_name ,avg(grade) from student
    inner join score on student.id=stu_id group by c_name;
-- 查询计算机成绩低于95的学生信息
select student.* from student inner join
    score on student.id=stu_id where c_name='计算机' and grade>95;
-- 查询同时参加计算机和英语考试的学生的信息
select stu.*
from score
inner join (select student.* from student
    inner join score on student.id=stu_id where c_name='计算机') stu
on stu_id=stu.id where c_name='英语';
-- 从student表和score表中查询出学生的学号，然后合并查询结果
select * from student
inner join score
on student.id=stu_id;
-- 查询姓张或者姓王的同学的姓名、院系和考试科目及成绩
select stu.name,stu.department,score.c_name,score.grade
from score inner join
   student stu on stu.id=stu_id
where stu.name like '张%' or stu.name like '王';

select distinct stu.name,stu.department,score.c_name,score.grade
from score inner join
    (select student.* from student inner join
        score s on student.id=s.stu_id where name like '张%' or '王%') stu
on stu_id=stu.id;
-- 查询都是湖南的学生的姓名、年龄、院系和考试科目及成绩
select  stu.name,2020-stu.birth+1 as year,stu.department,s.c_name,s.grade
from student stu
inner join score s
on stu.id=s.stu_id where stu.address like '湖南%';
...
inner join (select avg(sal) avs,emp.deptno from emp
    inner join dept on emp.deptno=dept.deptno
group by emp.deptno) av on emp.deptno=av.deptno where emp.sal>av.avs;
-- 取得部门中(所有人)平均薪水的等级

-- 取得部门中(所有人)薪水的平均等级
-- 取得平均薪水最高的部门的编号
select dept.deptno from dept inner join(
select avg(sal) avs,emp.deptno,dept.dname from emp
inner join dept on emp.deptno=dept.deptno group by emp.deptno)av 
on dept.deptno=av.deptno where av.avs=max(avs);
-- 求平均薪水的等级最高的部门的部门名称
